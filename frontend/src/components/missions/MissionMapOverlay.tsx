...
export function MissionMapOverlay({
  ...
  const tooltipMission = tooltip ? missions.find(x => x.id === tooltip.id) : null

  return (
    <div
      ref={mapContainerRef}
      className={`relative aspect-[4/3] md:aspect-auto overflow-hidden rounded-lg md:flex-1 md:min-h-0${cursorClass}`}
      style={{ touchAction: 'none', background: 'radial-gradient(ellipse at center, #c8cbc8 0%, #c8cbc8 97%, #f0f2f0 100%)' }}
        ...
        return (
          <div className="absolute pointer-events-none z-50"
            ...
            <div className="bg-bc-dark border border-bc-border rounded px-2.5 py-1.5 shadow-xl">
              <p className="font-mono text-xs font-bold text-bc-text">{tooltipMission.title}</p>
              <p className={`font-mono text-xs mt-0.5 ${STATUS_COLORS[tooltipMission.status].split(' ')[0]}`}>{STATUS_LABELS[tooltipMission.status]}</p>
            </div>
